...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (rows.length < 2) return { statusCode: 200, headers: CORS, body: '[]' };
    ...
    if (iCliente < 0 || iLat < 0 || iLng < 0) {
      return { statusCode: 500, headers: CORS, body: JSON.stringify({ error: 'Columnas no encontradas' }) };
    }
    const clientes = rows.slice(1)
      ...
      .filter(c => c.nombre && !isNaN(c.lat) && isFinite(c.lat) && !isNaN(c.lng) && isFinite(c.lng));
    return { statusCode: 200, headers: CORS, body: JSON.stringify(clientes) };
  } catch (err) {
